...
void guix_input_touch_callback(const struct device *dev, uint32_t row, uint32_t column, bool pressed)
{
	GX_EVENT event = {
		.gx_event_sender = 0,
		.gx_event_target = 0,
...
void guix_key_callback(const struct device *dev, uint32_t row, uint32_t column, bool pressed)
{
	GX_EVENT event = {
		.gx_event_sender = 0,
		.gx_event_target = 0,
...
{
	const struct device *dev_touch = device_get_binding("touch_cyttsp5");
	if (dev_touch != NULL) {
		kscan_config(dev_touch, guix_input_touch_callback);
	}

	const struct device *dev_key = device_get_binding("buttons");
	if (dev_key != NULL) {
