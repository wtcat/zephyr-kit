...
static inline int z_vrfy_counter_set_value(const struct device *dev,
					   uint32_t ticks)
{
	Z_OOPS(Z_SYSCALL_DRIVER_COUNTER(dev, set_value));
...
static inline int z_vrfy_counter_set_alarm_wk_rpt(const struct device *dev,
						   uint8_t chan_id,
						   const struct counter_alarm_wk_rpt_cfg *alarm_cfg)
{
	...
	Z_OOPS(Z_SYSCALL_VERIFY_MSG(cfg_copy.callback == 0,
				    "callbacks may not be set from user mode"));
	return z_impl_counter_set_alarm_wk_rpt((const struct device *)dev,
						(uint8_t)chan_id,
