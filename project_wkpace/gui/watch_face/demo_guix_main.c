...
//mirror_obj_t *wf_mirror_obj =NULL;
...
VOID start_status_menu_animation(GX_WINDOW *window)
{
    ...
    slide_animation_info.gx_animation_range_limit_type = GX_ANIMATION_LIMIT_STYLE_VERTICAL_DOWN;
...
VOID tmp_disp_draw(GX_WINDOW* widget)
{
	...
	gx_brush_define(&current_context->gx_draw_context_brush, brush_color, 0, GX_BRUSH_ROUND);
...
UINT guix_main(UINT disp_id, struct guix_driver *drv)
{
    ...
    gx_studio_display_configure(MAIN_DISPLAY, drv->setup, 
    0, MAIN_DISPLAY_THEME_1, &root);

    watch_face_mgr_init(drv->map_base);

    gx_studio_named_widget_create("root_window", (GX_WIDGET *) root,
                                    (GX_WIDGET **) &p_main_screen);//create empty watch face window.
    
    gx_studio_named_widget_create("window", GX_NULL,
                                    NULL);//create empty watch face window.
    ...
    gx_widget_show(root); //Show the root window to make it and patients screen visible.
...
UINT main_disp_handler(GX_WINDOW* window, GX_EVENT* event_ptr)
{
    ...
    {
    case GX_EVENT_SHOW:
        gx_system_timer_start(&root_window.root_window_watch_face_window, CLOCK_TIMER, 50, 50);
        break;
    case GX_EVENT_HIDE:
        gx_system_timer_stop(&root_window.root_window_watch_face_window, CLOCK_TIMER);
        break;
    case GX_EVENT_TIMER:
...
VOID main_disp_draw(GX_WINDOW* widget)
{
    if ((!wf_mirror_mgr.wf_use_mirror_or_not) || 
        ...
        watch_face_theme_show(widget);

        #if 0  //testing: copy data at once when screen refresh.
...
UINT root_event(GX_WINDOW* window, GX_EVENT* event_ptr)
{
    ...
    case GX_EVENT_SHOW:
        ...
        break;
    case GX_EVENT_HIDE:
        mirror_obj_delete(&wf_mirror_mgr.wf_mirror_obj);             //  reset obj's canvas data
        break;
    #if 1
    ...
    case GX_EVENT_PEN_DRAG:
    ...
    #else
        //testing: copy data at once when screen refresh.
        if ((!wf_mirror_mgr.wf_use_mirror_or_not) && 
                (wf_mirror_mgr.wf_mirror_obj != NULL)){
            wf_mirror_mgr.wf_use_mirror_or_not = 1;
        }
        ...
        if ((root_window_enter_pen_down) && (!enter_h) && (!enter_v)){
            ...
            if ((delta_x > 5) || (delta_y > 5))
            {
                if (delta_y > delta_x)
                {
                    start_status_menu_animation(window);
                    enter_h = 1;
                }
                else {
