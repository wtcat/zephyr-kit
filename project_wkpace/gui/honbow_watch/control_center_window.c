...
typedef struct children_module {
	CC_ICON_T night_mode;
	bool night_mode_on;
	CC_ICON_T bright_screen_switch;
	bool auto_bright_screen;
	CC_ICON_T disturb_mode;
	CC_ICON_T bright_ness;
...
void brightness_adjust_draw(GX_WIDGET *widget)
{
	...
	gx_prompt_draw(&prompt_for_brightness);
}
void control_center_window_init(GX_WIDGET *parent) // add child module for control center window
{
	...
	if (children_module.night_mode_on == true) {
		...
		custom_icon_create(&children_module.night_mode, NIGHT_MODE_ICON_ID, parent, &child, GX_COLOR_ID_GRAY,
						   GX_PIXELMAP_ID_CONTROL_CENTER_NIGHT);
	}
	...
	custom_icon_create(&children_module.disturb_mode, DISTURB_MODE_ICON_ID, parent, &child, GX_COLOR_ID_HONBOW_ORANGE,
	...
	custom_icon_create(&children_module.set_up, SET_UP_ICON_ID, parent, &child, GX_COLOR_ID_GRAY,
	...
	gx_prompt_text_color_set(&children_module.date, GX_COLOR_ID_WHITE, GX_COLOR_ID_WHITE, GX_COLOR_ID_WHITE);
	...
	gx_prompt_create(&children_module.time, NULL, parent, 0,
					 GX_STYLE_TRANSPARENT | GX_STYLE_TEXT_LEFT | GX_STYLE_BORDER_NONE, GX_ID_NONE, &child);
	gx_prompt_text_color_set(&children_module.time, GX_COLOR_ID_WHITE, GX_COLOR_ID_WHITE, GX_COLOR_ID_WHITE);
	gx_prompt_font_set(&children_module.time, GX_FONT_ID_SIZE_30);

	// battey prompt
	gx_utility_rectangle_define(&child, 230, 14, 236 + 74 - 1, 14 + 28 - 1);
	gx_prompt_create(&children_module.battery_tips, NULL, parent, 0,
					 GX_STYLE_TRANSPARENT | GX_STYLE_TEXT_LEFT | GX_STYLE_BORDER_NONE, GX_ID_NONE, &child);
	gx_prompt_text_color_set(&children_module.battery_tips, GX_COLOR_ID_WHITE, GX_COLOR_ID_WHITE, GX_COLOR_ID_WHITE);
	...
	gx_icon_create(&children_module.bt_link_status, NULL, parent, GX_PIXELMAP_ID_CONTROL_CENTER_LINK,
				   GX_STYLE_BORDER_NONE, GX_ID_NONE, 12, 20);
	...
	gx_utility_rectangle_define(&child, 120, 10, 120 + 80 - 1, 13);
	custom_icon_create(&children_module.line_tips, GX_ID_NONE, parent, &child, GX_COLOR_ID_WHITE, 0);
	...
	gx_prompt_create(&children_module.operation_tips, NULL, parent, 0, GX_STYLE_TEXT_CENTER | GX_STYLE_BORDER_NONE,
					 GX_ID_NONE, &child);
	gx_prompt_text_color_set(&children_module.operation_tips, GX_COLOR_ID_WHITE, GX_COLOR_ID_WHITE, GX_COLOR_ID_WHITE);
	gx_prompt_font_set(&children_module.operation_tips, GX_FONT_ID_SIZE_30);
	gx_widget_fill_color_set(&children_module.operation_tips, GX_COLOR_ID_BLACK, GX_COLOR_ID_BLACK, GX_COLOR_ID_BLACK);
...
UINT control_center_window_event(GX_WINDOW *window, GX_EVENT *event_ptr)
{
	...
	case GX_SIGNAL(NIGHT_MODE_ICON_ID, GX_EVENT_CLICKED): {
		if (children_module.night_mode_on == true) {
			custom_icon_change_bg_color(&children_module.night_mode, GX_COLOR_ID_GRAY);
	...
	case GX_SIGNAL(BRIGHT_SCREEN_ON_ICON_ID, GX_EVENT_CLICKED): {
		if (children_module.auto_bright_screen == true) {
			custom_icon_change_bg_color(&children_module.bright_screen_switch, GX_COLOR_ID_GRAY);
