...
VOID thumb_widget_draw(GX_WIDGET *widget)
{
	...
	if (map != NULL) {
		...
		gx_canvas_pixelmap_draw(left, top, map);
		gx_widget_border_draw(widget, GX_COLOR_ID_SILVERY_GRAY, GX_COLOR_ID_SILVERY_GRAY, GX_COLOR_ID_SILVERY_GRAY,
							  false);
	}
}
...
VOID wf_list_column_create(GX_HORIZONTAL_LIST *list, GX_WIDGET *widget, INT index)
{
	...
	if (!result) {
		...
		gx_widget_fill_color_set(&column->widget, GX_COLOR_ID_BLACK, GX_COLOR_ID_BLACK, GX_COLOR_ID_BLACK);
		...
		gx_prompt_text_color_set(&column->theme_prompt, GX_COLOR_ID_MILK_WHITE, GX_COLOR_ID_MILK_WHITE,
		...
		gx_widget_create(&column->thumb, NULL, &column->widget, GX_STYLE_ENABLED | GX_STYLE_BORDER_THIN, GX_ID_NONE,
						 &childsize);
		gx_widget_fill_color_set(&column->thumb, GX_COLOR_ID_BLUE, GX_COLOR_ID_BLUE, GX_COLOR_ID_BLUE);
	...
	case GX_EVENT_PEN_UP:
		if (list->gx_widget_status & GX_STATUS_OWNS_INPUT) {
			...
			if (list->gx_horizontal_list_pen_index >= 0 && list->gx_horizontal_list_snap_back_distance == 0) {
				/* test to see if pen-up is over same child widget as
				 * pen-down
				 */
				...
				screen_switch(list->gx_widget_parent, (GX_WIDGET *)&root_window.root_window_home_window);
